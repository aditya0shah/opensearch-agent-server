# Wiring ml-commons to the agent server `/generate_dsl` endpoint

These are example registrations for connecting an OpenSearch cluster's
agentic-search path to the agent server's `/generate_dsl` endpoint. They are
**configuration, not code** — JSON you POST to a running cluster's REST API.
Fill in your own agent-server URL and credential before using them.

## What's here

| File | What it is |
|---|---|
| `01_connector.json` | ml-commons **connector** — describes the HTTP call to `/generate_dsl`. |
...
`/generate_dsl` endpoint (`src/server/dsl_routes.py`), which these point at.
...
response in `dataAsMap`, leaving `result` empty. The connector's built-in
`post_process_function: connector.post_process.mlcommons.passthrough` copies
`output[0].result` from the endpoint's `inference_results` envelope into
`ModelTensor.result`, which is why `/generate_dsl` returns that envelope shape.
