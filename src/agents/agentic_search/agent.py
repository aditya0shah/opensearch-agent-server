...
class AgenticSearchAgent:
    ...
    def _select_strategy(context: dict[str, Any]) -> GenerationStrategy:
        ...
        The single-candidate case deliberately does not go through the multi-template
        path: one call that both chooses and fills is measurably less precise at
        filling than the focused single-template prompt, so a request that needs no
        choice should not pay for one. Template mode stays opt-in and additive —
        callers that send no template keys are unaffected, and an existing caller
        sending a scalar ``template_id`` keeps its current behavior exactly.
        """
