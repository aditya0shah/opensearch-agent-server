...
Selected when a request carries **more than one** candidate in
``context.template_ids``. One forced tool call both picks a template and fills its
parameters: every candidate's params are merged into one flat object, namespaced by
template id and all optional, alongside a required ``template_id`` choice. The chosen
template's params are then validated against its real model (which restores the
required-ness and enum constraints an all-optional merge cannot express) and rendered
by OpenSearch, exactly as in the single-template path.

Why one call rather than routing first and filling second: routing separately costs an
extra sequential round trip (~75% more p50 in benchmarking) that does not shrink as
templates get more specific, while the accuracy it buys comes from filling with the
focused single-template prompt. So this strategy deliberately avoids the combined
prompt whenever it can:

- one candidate (after filtering) -> delegate to the single-template strategy, whose
  prompt fills more precisely;
- several candidates -> the combined call, which is the only case that needs a choice.

Candidates are pre-filtered to the request's index and capped, because the merged
schema grows with the candidate count (roughly 5k tokens per template for broad
templates), and an unbounded set would dominate the prompt.
...
def _prefix_for(template_id: str) -> str:
    ...
    Namespacing is load-bearing: with bare names, identically-named slots from
    different candidates collide on a single merged property and the model fills the
    wrong template's semantics into it. Use :func:`_prefixes_for` to get prefixes that
    are also guaranteed unique across a candidate set.
    """
...
class MultiTemplateFillStrategy:
    ...
    def generate(self, request: GenerationRequest) -> dict[str, Any]:
        ...
        # One candidate needs no choice, and the single-template prompt fills more
        # precisely than the combined one, so hand it over.
        if len(candidates) == 1:
    ...
    def _resolve(self, ids: list[str], request: GenerationRequest) -> list[TemplateSchema]:
