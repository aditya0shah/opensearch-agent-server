...
# ``template_id`` (which is also the core ``_scripts`` template name). The scoped
# name disambiguates from core's ``_scripts`` "search templates". Must match the
# ml-commons CommonValue.ML_AGENTIC_SEARCH_TEMPLATES_INDEX constant.
...
# integer fields (e.g. ``size``). ``float``/``double`` stay float where a decimal is
# expected. Order matters in the union: int first so integer inputs stay ints.
