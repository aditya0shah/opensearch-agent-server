...
- `multi_template_fill` generation strategy for `agentic_search`: pass several candidate templates as `context.template_ids` and one forced tool-choice call picks one and fills it. Each candidate is offered as its own tool (its parameters are that template's fields), with a dedicated free-DSL fallback tool for "none fit". Candidates are filtered to the request's index and capped; a request naming only one template still uses `template_fill`
