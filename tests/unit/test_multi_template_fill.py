...
class _FakeClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.rendered = rendered if rendered is not None else {"query": {"match_all": {}}}
    ...
    def render_search_template(self, *, id: str, body: dict[str, Any]) -> dict[str, Any]:
...
class _FakeCache:
    ...
    def __init__(self, schemas: dict[str, TemplateSchema], missing: set[str] | None = None):
...
def _schema(tid: str, index: str, params: dict[str, Any], desc: str = "") -> TemplateSchema:
...
CATALOG = {"lex_query": {"type": "string", "description": "text"},
           "price_max": {"type": "number", "description": "max price"}}
LISTINGS = {"lex_query": {"type": "string", "description": "text"},
            "beds_min": {"type": "number", "description": "min beds"}}


def _request(context: dict[str, Any], client: Any, model: Any = None) -> GenerationRequest:
    ...
    def __init__(self, payload: dict[str, Any]) -> None:
        ...
        class _Client:
            def converse_stream(self, **kwargs):
                ...
                return {
                    "stream": [
                        {"contentBlockDelta": {"delta": {"toolUse": {"input": json.dumps(outer._payload)}}}},
                    ]
                }
...
def test_distinct_ids_preserve_order_and_dedupe():
    assert _distinct_template_ids({"template_ids": ["b", "a", "b"], "template_id": "a"}) == ["b", "a"]
...
def test_single_surviving_candidate_delegates_to_single_template_path():
    ...
    strat = MultiTemplateFillStrategy(single=single, fallback=fallback, schema_cache=cache)
...
def test_index_filter_reducing_to_one_candidate_delegates():
    ...
    single, fallback = _RecordingSingle(), _RecordingFallback()
    cache = _FakeCache(
        {"t1": _schema("t1", "idx-a", CATALOG), "t2": _schema("t2", "idx-OTHER", LISTINGS)}
    )
    strat = MultiTemplateFillStrategy(single=single, fallback=fallback, schema_cache=cache)
...
def test_none_choice_routes_to_fallback():
    strat, request, client, _, fallback = _two_candidate_strategy({"template_id": "none"})
    strat.generate(request)
    assert fallback.calls == 1
    assert client.render_calls == []
...
def test_unknown_choice_routes_to_fallback():
    strat, request, client, _, fallback = _two_candidate_strategy({"template_id": "nope"})
    strat.generate(request)
    assert fallback.calls == 1
    assert client.render_calls == []
...
def test_tool_schema_declares_abstain_before_choice_and_namespaces_params():
    single, fallback = _RecordingSingle(), _RecordingFallback()
    cands = [_schema("t1", "idx-a", CATALOG, "catalog"), _schema("t2", "idx-a", LISTINGS, "listings")]
    strat = MultiTemplateFillStrategy(
        single=single, fallback=fallback, schema_cache=_FakeCache({})
    )
...
def test_nested_prefix_ids_do_not_leak_params():
    ...
    single, fallback = _RecordingSingle(), _RecordingFallback()
    cache = _FakeCache(
        {"x": _schema("x", "idx-a", CATALOG), "x__y": _schema("x__y", "idx-a", LISTINGS)}
    )
    strat = MultiTemplateFillStrategy(single=single, fallback=fallback, schema_cache=cache)
    _, name_map = strat._build_tool_spec([cache.get("x", None), cache.get("x__y", None)])
    ...
    client = _FakeClient()
    strat.generate(_request({"template_ids": ["x", "x__y"]}, client, _FakeModel(payload)))
...
def test_candidate_named_none_is_still_selectable():
    ...
    single, fallback = _RecordingSingle(), _RecordingFallback()
    cache = _FakeCache(
        {"none": _schema("none", "idx-a", CATALOG), "t2": _schema("t2", "idx-a", LISTINGS)}
    )
    strat = MultiTemplateFillStrategy(single=single, fallback=fallback, schema_cache=cache)
    ...
    client, model = _FakeClient(), _FakeModel({"template_id": "none", "none__lex_query": "x"})
